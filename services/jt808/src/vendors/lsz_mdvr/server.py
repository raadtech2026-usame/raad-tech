...
vendor's own protocol/dispatcher/handlers stack onto the parent package's **unmodified**
`connection`/`session`/`events` layers — the concrete proof that ADR-0009's "only the protocol
adapter differs" claim holds in code, not just in the architecture record.

Structurally mirrors `src.server.Jt808Server` closely (same signal handling, same start/stop/
serve_forever shape, same "malformed frame logged and dropped before dispatch" behavior) — kept
as a separate class rather than a shared base, since the two servers' constructors wire genuinely
different handler sets and neither is a strict subset of the other; a shared base class would buy
little beyond what already-shared `ConnectionManager`/`DeviceSessionManager` provide.
...
from src.vendors.lsz_mdvr.protocol.exceptions import MdvrProtocolError
