...
class FrameTooLargeError(_SharedFrameTooLargeError):
    """Raised when buffered, undelimited bytes exceed `max_frame_size` — a malformed or
    hostile peer sending an unbounded stream without a closing delimiter. Subclasses the shared
    `connection.errors.FrameTooLargeError` (device-gateway multi-vendor architecture) so
    `connection.Connection`'s read loop can catch it without importing this vendor's own
    protocol package; the caller still sees this JT/T-808-specific, descriptively-named type."""
