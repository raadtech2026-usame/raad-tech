...
class DriverApplicationServiceReadTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_list_drivers_returns_summary_dtos_for_all_drivers(self) -> None:
        ...
        self.assertTrue(all(isinstance(dto, DriverSummaryDTO) for dto in page.data))
        self.assertEqual(
            sorted(dto.license_no for dto in page.data), ["Driver One", "Driver Two"]
        )
...
class DriverApplicationServicePaginationTests(unittest.IsolatedAsyncioTestCase):
    async def test_list_drivers_paginates_and_reports_total(self) -> None:
        ...
        page = await service.list_drivers(
            ListDriversQuery(page_request=OffsetPageRequest(page=1, page_size=2)),
            uow=uow,
        )
        ...
        second_page = await service.list_drivers(
            ListDriversQuery(page_request=OffsetPageRequest(page=2, page_size=2)),
            uow=uow,
        )
    ...
    async def test_list_drivers_filters_by_status(self) -> None:
        ...
        self.assertEqual(page.data[0].license_no, "DISABLED-1")
    ...
    async def test_list_drivers_sorts_descending_by_license_no(self) -> None:
        ...
        self.assertEqual(
            [dto.license_no for dto in page.data], ["Gamma", "Beta", "Alpha"]
        )
