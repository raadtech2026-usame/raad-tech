...
versioned base path `/api/v1`. Every router is currently empty (Phase 4.2 scope) — this file
only establishes correct placement so real endpoints land under the right prefix when each
module's application/domain layers are implemented.
"""
...
from raad.modules.iam.api.routers import auth_router
...
api_router.include_router(vehicles_router, prefix="/vehicles", tags=["fleet"])  # fleet_device (C3)
api_router.include_router(devices_router, prefix="/devices", tags=["fleet"])

api_router.include_router(students_router, prefix="/students", tags=["transport-ops"])  # transport_ops (C4)
...
api_router.include_router(tracking_router, prefix="/tracking", tags=["tracking"])  # tracking (C5)
...
api_router.include_router(billing_router, prefix="/billing", tags=["billing"])  # billing (C8)

api_router.include_router(reports_router, prefix="/reports", tags=["reporting"])  # reporting (C9)

api_router.include_router(admin_router, prefix="/admin", tags=["admin"])  # platform_audit (C10)
