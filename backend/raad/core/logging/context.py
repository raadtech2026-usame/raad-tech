...
def bind_context(**fields: str | None) -> dict[str, Token]:
    """Sets any of request_id/correlation_id/principal_id/role/org_id for the current
    context. Returns reset tokens so the caller can restore prior values (see
    `reset_context`) — used by middleware to unbind at the end of a request."""
